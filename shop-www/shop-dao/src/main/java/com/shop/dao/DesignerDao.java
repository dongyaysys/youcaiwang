...
public interface DesignerDao {
    ...
    @Select("select id,image,name,designser_top,designser_centre," +
            "designser_bottom  from dm_designer where is_selection=1 and version=1 limit 5")
    public List<DesignerQuery> showHotDesignders();

    @Select("SELECT id,recent_image FROM dm_designer where version=1 ORDER BY create_date desc limit 5")
    public List<DesignerQuery> showRecentDesignders();

    public List<DesignerQuery> showPopularDesignders();

    @Select("select id,name,image_detail,introduction from dm_designer where id=#{id} and version=1")
    public Designer showDesignerDetail(@Param("id") Integer id);

    @Select("select id,heading,name,fristPinyin(name) as fristPinyin from dm_designer where version=1 ORDER BY fristPinyin")
